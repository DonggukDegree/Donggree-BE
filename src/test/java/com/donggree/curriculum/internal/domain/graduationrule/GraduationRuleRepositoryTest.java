...
@DataJpaTest
@Import({QueryDslConfig.class, GraduationRuleRepositoryImpl.class})
class GraduationRuleRepositoryTest {
    ...
    void 필터가_없으면_course_type_그다음_rule_type_id_순으로_전체_조회한다() {
        List<GraduationRuleProjection> result = customRepository.search(null, null);
    ...
    void 조회_결과에_rule_type을_조인해_typeName과_courseType을_채운다() {
        GraduationRuleProjection major =
                customRepository.search(List.of(majorTypeId), null).get(0);
    ...
    void ruleTypeId로_필터링한다() {
        List<GraduationRuleProjection> result = customRepository.search(List.of(majorTypeId), null);
    ...
    void courseType으로_필터링한다() {
        List<GraduationRuleProjection> result = customRepository.search(null, List.of(CourseType.LIBERAL_ARTS));
    ...
    void courseType을_여러_개_지정하면_OR로_정렬되어_조회한다() {
        List<GraduationRuleProjection> result =
                customRepository.search(null, List.of(CourseType.FIRST_MAJOR, CourseType.LIBERAL_ARTS));
