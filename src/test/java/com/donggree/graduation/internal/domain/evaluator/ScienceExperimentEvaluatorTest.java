...
class ScienceExperimentEvaluatorTest extends EvaluatorTestSupport {
    ...
    void 실험_과목을_1개_이상_이수하면_충족이다() {
        ...
        var cls = Map.of("SCI001",
                classification(1L, com.donggree.curriculum.CourseType.ACADEMIC_FOUNDATION, "실험", "물리"));
        EvaluationContext ctx = context(transcript(3, 4.0, records), cls);

        assertThat(evaluator.evaluate(rule("{\"minCount\": 1}"), ctx).satisfied()).isTrue();
    }

    @Test
    void 실험_과목이_없으면_미충족이다() {
        var records = List.of(passed("SCI002", "물리학개론", 3, "2023-1"));
        var cls = Map.of("SCI002",
                classification(2L, com.donggree.curriculum.CourseType.ACADEMIC_FOUNDATION, "개론", "물리"));
        EvaluationContext ctx = context(transcript(3, 4.0, records), cls);

        assertThat(evaluator.evaluate(rule("{\"minCount\": 1}"), ctx).satisfied()).isFalse();
    }
    ...
    private GraduationRuleView rule(String config) {
        return new GraduationRuleView(1L, "SCIENCE_EXPERIMENT", RuleCategory.LIBERAL,
                "실험 교과목 중 1과목을 필수 선택해야 합니다.", config);
    }
}
