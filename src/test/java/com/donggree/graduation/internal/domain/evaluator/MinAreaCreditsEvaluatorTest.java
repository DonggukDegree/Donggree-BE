...
class MinAreaCreditsEvaluatorTest extends EvaluatorTestSupport {
    ...
    void 공통교양_학점이_최소_이상이면_충족이다() {
        ...
        GraduationRuleView rule = rule("{\"courseType\": \"COMMON_GENERAL\", \"subCategories\": null, \"minCredits\": 17}");
    ...
    void subCategory_필터링으로_기본소양만_집계한다() {
        ...
        GraduationRuleView rule = rule(
                "{\"courseType\": \"ACADEMIC_FOUNDATION\", \"subCategories\": [\"기본소양\"], \"minCredits\": 3}");
    ...
    void 이수_실패_과목은_학점에_포함되지_않는다() {
        ...
        GraduationRuleView rule = rule("{\"courseType\": \"FIRST_MAJOR\", \"subCategories\": null, \"minCredits\": 60}");

        assertThat(evaluator.evaluate(rule, ctx).satisfied()).isFalse();
    }

    private GraduationRuleView rule(String config) {
        return new GraduationRuleView(1L, "MIN_AREA_CREDITS", RuleCategory.LIBERAL,
                "테스트 규칙", config);
    }
}
