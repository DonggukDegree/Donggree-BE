...
class PrerequisiteEvaluatorTest extends EvaluatorTestSupport {
    ...
    void 선이수_과목을_이전_학기에_이수했으면_충족이다() {
        var records = List.of(
                passed("CSE1001", "기초프로그래밍", 3, "2023-1"),
                passed("CSE2001", "자료구조", 3, "2023-2")
        );
    ...
    void 선이수를_나중_학기에_수강하면_미충족이다() {
        var records = List.of(
                passed("CSE2001", "자료구조", 3, "2023-1"),
                passed("CSE1001", "기초프로그래밍", 3, "2023-2")
        );
        EvaluationContext ctx = contextNoClassification(transcript(6, 4.0, records));
    ...
    void 조건_필드가_맞지_않는_학생에게는_규칙이_적용되지_않는다() {
        String configWithCondition =
                "{\"targetCourseName\": \"EAS1\", \"prerequisiteCourseName\": \"Basic EAS\", " +
        ...
        assertThat(evaluator.evaluate(rule(configWithCondition), ctx).satisfied()).isTrue();
    }
    ...
    void 조건_필드가_맞는_학생에게는_선이수_규칙이_적용된다() {
        ...
        assertThat(evaluator.evaluate(rule(configWithCondition), ctx).satisfied()).isFalse();
    }

    private GraduationRuleView rule(String config) {
        return new GraduationRuleView(1L, "PREREQUISITE", RuleCategory.MAJOR,
                "자료구조 이전에 기초프로그래밍을 선이수해야 합니다.", config);
    }
}
