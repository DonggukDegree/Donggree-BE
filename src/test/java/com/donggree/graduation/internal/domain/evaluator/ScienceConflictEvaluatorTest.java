...
class ScienceConflictEvaluatorTest extends EvaluatorTestSupport {
    ...
    void 같은_subject_domain의_실험과_개론을_이수하면_미충족이다() {
        var records = List.of(
                passed("SCI001", "일반물리학및실험1", 3, "2023-1"),
                passed("SCI002", "물리학개론", 3, "2023-2")
        );
        var cls = Map.of(
    ...
    private GraduationRuleView rule(String config) {
        return new GraduationRuleView(1L, "SCIENCE_CONFLICT", RuleCategory.LIBERAL,
                "동일한 실험 교과목과 개론 교과목을 이수할 수 없습니다.", config);
    }
}
